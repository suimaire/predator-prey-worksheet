      ...
      <div className="page-shell">
        ...
        <section className="worksheet" aria-label="포식자와 피식자 학습 활동">
          ...
          <article className="activity-card" id="activity-3">
            ...
            <div className="variable-card"><span>이번에 바꿀 변수</span><b><i>γ</i> 포식자 자연 사망률</b><p>다른 조건은 그대로 두고 이 값만 증가시킵니다.</p></div>
            <div className="questions">
              <fieldset className="question"><legend><span>7</span>포식자의 자연 사망률 γ를 증가시키면 가장 직접적으로 예상되는 변화는?</legend><RadioCards name="gamma" value={answers.gammaPrediction} onChange={(value) => setField('gammaPrediction', value)} options={[{value:'A',label:'A',detail:'포식자가 더 오래 산다.'},{value:'B',label:'B',detail:'포식자가 더 빠르게 감소하는 경향이 나타난다.'},{value:'C',label:'C',detail:'피식자의 자연 증가율이 직접 감소한다.'},{value:'D',label:'D',detail:'환경수용력이 증가한다.'}]} /><Feedback show={answers.revealed} correct={checks[5]}>사망률 증가는 포식자의 손실을 직접 키우므로 포식자가 더 빠르게 감소하는 경향을 예상할 수 있습니다.</Feedback></fieldset>
              <section className="protocol" aria-labelledby="protocol-title"><div className="protocol-heading"><span aria-hidden="true">↻</span><div><small>실험 절차</small><h3 id="protocol-title">한 번에 하나의 변수만 바꾸기</h3></div></div><ol><li><b>1</b>기본값에서 Run 실행</li><li><b>2</b>그래프의 형태 관찰</li><li><b>3</b>다른 조건은 그대로 유지</li><li><b>4</b>γ만 증가</li><li><b>5</b>다시 Run</li><li><b>6</b>두 결과 비교</li></ol></section>
              <fieldset className="question"><legend><span>8</span>실험 결과를 기록하세요.</legend><div className="observation-grid"><div className="table-head">관찰 항목</div><div className="table-head">결과</div><label>포식자 개체수</label><select value={answers.predatorObservation} onChange={(e) => setField('predatorObservation', e.target.value)}><option value="">선택</option><option>증가</option><option>감소</option><option>큰 차이 없음</option></select><label>피식자 개체수</label><select value={answers.preyObservation} onChange={(e) => setField('preyObservation', e.target.value)}><option value="">선택</option><option>증가</option><option>감소</option><option>큰 차이 없음</option></select><label htmlFor="oscillation">진동의 형태</label><textarea id="oscillation" rows={3} value={answers.oscillationObservation} onChange={(e) => setField('oscillationObservation', e.target.value)} placeholder="주기, 진폭, 정점 위치 등을 비교하세요." /></div>{answers.revealed && <div className="model-answer"><b>관찰 결과는 설정과 실행마다 달라질 수 있어요</b><p>일반적으로 포식자 사망률이 커지면 포식 압력이 약해져 포식자는 감소하고 피식자는 증가하는 경향을 보일 수 있습니다. 확률적 모형이므로 한 번의 결과보다 반복 경향을 비교하세요.</p></div>}</fieldset>
              <fieldset className="question core-question"><legend><span>9</span>포식자의 사망률만 변화시켰는데 왜 피식자의 그래프도 변했을까요?</legend><p className="question-hint">이 활동의 핵심 문항입니다. 두 개체군을 서로 연결된 시스템으로 생각해 보세요.</p><label className="text-field"><textarea rows={5} value={answers.indirectReason} onChange={(e) => setField('indirectReason', e.target.value)} placeholder="포식 압력이라는 말을 사용해 설명해 보세요." /></label>{answers.revealed && <div className="model-answer emphasized"><b>핵심 해설</b><p>포식자와 피식자는 독립된 개체군이 아니라 서로 영향을 주는 시스템입니다. 포식자의 사망률 변화는 포식 압력을 변화시키고, 그 결과 피식자의 개체수에도 간접적인 변화가 나타납니다.</p></div>}</fieldset>
            </div>
          </article>
